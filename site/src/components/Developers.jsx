...
const ENDPOINTS = [
  ["GET", "/api/state", "Every seller, its tier, score, and how the agent would pay it right now."],
  ["GET", "/api/feed", "The journal. One line per transaction, newest first, never edited."],
  ["POST", "/api/run", "Runs one real screening from the demo wallet and returns the updated record."],
];
    ...
    <section className="dev" id="developers">
      <div className="dev-inner">
        ...
        <p className="dev-sub" data-reveal style={{ "--d": "80ms" }}>
          Iterum is a small Python library over Sibyl Memory. Record what a
          counterparty did, then ask what terms it has earned.
        </p>
